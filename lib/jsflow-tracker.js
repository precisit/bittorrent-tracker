...
function jsFlowTracker (client, announceUrl, opts) {
  ...
  jsFlow.addHandler('webrtc_offer', self._onWebRtcOffer.bind(self))
  jsFlow.addHandler('webrtc_answer', self._onWebRtcAnswer.bind(self))
  jsFlow.addHandler('webrtc_ice', self._onWebRtcIce.bind(self))
}
...
jsFlowTracker.prototype._onData = function (payload, from) {
  ...
  if (Array.isArray(payload.peers)) {
    payload.peers.forEach(function (peerId) { //For all peers we receive - Open a WebRTC peer connection

      console.log('Will handle connection for peer: ', peer);
